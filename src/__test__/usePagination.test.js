...
const PROPS_WHEN_WITHOUT_TOTAL_PAGE = {
  numOfPage: 5,
}
...
describe('usePagination', () => {
  ...
  it('totalPage 값이 주어지지 않은 경우에도 currentPage는 1로 초기화되어야 한다', () => {
    const { result } = renderHook(() =>
      usePagination(PROPS_WHEN_WITHOUT_TOTAL_PAGE)
    )
  ...
  it('totalPage 값이 주어지지 않은 경우에 initialPage가 주어진다면 currentPage는 initialPage로 초기화되어야 한다.', () => {
    const { result } = renderHook(() =>
      usePagination({ ...PROPS_WHEN_WITHOUT_TOTAL_PAGE, initialPage: 6 })
    )

    expect(result.current.currentPage).toBe(6)
    expect(result.current.pageList).toStrictEqual([6])
  })
  ...
  it('initialPage가 주어진다면 currentPage는 initialPage로 초기화되어야 한다.', () => {
    const { result } = renderHook(() =>
      usePagination({ ...PROPS_WHEN_REMAINDER_OCCURS, initialPage: 8 })
    )

    expect(result.current.currentPage).toBe(8)
  })

  it('initialPage로 초기화된 이후 setTotalPage를 호출하여도 currentPage는 유지해야한다.', () => {
    const { result } = renderHook(() =>
      usePagination({ ...PROPS_WHEN_WITHOUT_TOTAL_PAGE, initialPage: 6 })
    )
    ...
    act(() => result.current.setTotalPage(15))

    expect(result.current.currentPage).toBe(6)
  })

  it('page가 변경될 경우 onPageChange callback을 호출해야한다.', () => {
    const mockCallback = jest.fn()

    const { result } = renderHook(() =>
      usePagination({ ...DEFAULT_PROPS, onPageChange: mockCallback })
    )
    ...
    for (let i = 1; i < 14; i++) {
      act(() => result.current.goNext())
      expect(result.current.currentPage).toBe(i + 1)
      expect(mockCallback).toBeCalledWith(i + 1)
    }
  })
})
